...
EXPECT_RED = {
    ...
    # 第35轮路M 收口：以下 2 个 R29/R30 边界测试是**死文件**，登记预期红而非删除。
    # 设计前提（"CCW 清零后 位与/位或/位非/幂次/除非/应当 等仍整词成 IDENTIFIER 并可作函数调用"）
    # 已被 R30 自身的 CCW 清零（184→0）自推翻：现实里 `位与`→`位`+`与`、`幂次`→`幂`+`次`、
    # `除非`→`除`+`非`、`应当`→`应`+`当`（已用 lexer 实证）。两文件还各有孤立 `捕获` 块
    # 缺 `尝试:`、且调用未定义函数名（位与/幂次/记录类型…）；自入库起从未 rc=0。
    # 词法层已由 tests/test_R29_CCW表分批精简_token.py / tests/test_R30_CCW保留10条通用化_token.py
    # 的 token 流用例钉住；运行期边界测试不可恢复。标记预期红使其可被门禁感知（将来若修复文件
    # 使之 rc=0，本条会反红，逼你移除登记），而非悄悄绿着。
    'test_R29_CCW精简边界.light': 'R29 死测试：CCW 清零后 位与/除非/应当 等被切分 + 孤立捕获块 + 未定义函数，运行期不可恢复，预期 rc!=0',
    'test_R30_CCW通用化边界.light': 'R30 死测试：CCW 清零后 位与/位非/幂次 等被切分 + 孤立捕获块 + 未定义函数，运行期不可恢复，预期 rc!=0',
