...
## 七、主线端到端实测：发现并修复第 2 个 bug（2026-09-09，集成验收阶段）

**现象**：四路合入后浏览器全链路实测，输入消息点击发送 → 服务端返回
`400：messages 缺少用户消息`，助手气泡不出现。

**根因**：`sendMessage()` 中顺序错误——第 192 行 `var msgs = history.slice()` 构造请求体时，
本轮用户消息尚未 push 到 history（push 在第 202 行，body 构造之后）。首次发送时 history 为空，
`messages: []` → 服务端校验失败。W1 查收时后端 agent API 未接（只验证了 501 错误路径），
此 bug 未暴露；端到端实测才发现。

**修复**：将"追加本轮 UI 与历史"（history.push 用户消息 + 空助手、addBubble、清空输入、
setStreaming）整体前移到"组装消息体"之前，使 `msgs = history.slice()` 包含本轮用户消息，
尾部空助手仍由去重循环剔除。提交 `64e1c94`。

**复验（浏览器全链路实测，mock 模式）**：
- 打开登录 URL（带 token）→ token 擦除、状态"已连接" ✅
- 输入"你好，请用三句话介绍你自己" → 发送 → 用户气泡（蓝色）出现、输入框清空 ✅
- 助手气泡逐字流式渲染："你好，我是 mock 模型的 web 服务回复" ✅
- 流式完成后停止按钮恢复、输入框可继续输入 ✅
- console 无 JS 错误 ✅