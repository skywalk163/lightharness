# -*- coding: utf-8 -*-
"""_antirun_t2_存储核心.py —— 第15轮 任务2（存储核心与JSON域）反跑判据

对象：lightharness/src/存储核心.light（字节级备份 -> 变异 -> 跑本路测试 -> 断红 -> 恢复 -> 断绿）
判据（3 项）：
  A=正常：内存后端写入->读取 round-trip 数据一致（无变异回归绿验证）
  B=边界：空存储单元批量读取返回空列表不报错（无变异回归绿验证）
  C=变异：取后端 的 NotFound 消息改错 -> 测试立红（恢复后绿）
恢复放在 try/finally 异常安全路径；恢复后 sha256 逐字节一致。
用法：python lightharness/_antirun_t2_存储核心.py（BASE 自定位）
退出码：全部通过 0；任一失败 1。
"""
import hashlib
import os
...
TARGET = os.path.join(BASE, "src", "存储核心.light")
TEST = os.path.join("examples", "test_存储核心.light")

C_OLD = "' is not registered (registered: \""
C_NEW = "' IS-NOT-REGISTERED (registered: \"  # antirun C"
...
def main():
    ...
    print("%s A/B 判据（内存后端 round-trip + 空单元批量读）回归绿 (rc=%d)" % ("PASS" if rc_green == 0 else "FAIL", rc_green))
