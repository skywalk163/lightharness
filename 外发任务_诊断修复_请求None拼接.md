...
> 1:1 复刻 DeepSeek Harness 的纯逻辑核心。本任务排查 CLI 单轮 / 交互调用时
>
> **必现**
>
> 的
...
> 门禁：
>
> `python 运行.py examples/test_XXX.light`
>
> 退出码 = 0 且断言全过；只跑定向 + 回归，不跑全量；
> 全程
>
> **不**
>
...
```
Remove-Item Env:HARNESS\_REPL -ErrorAction SilentlyContinue

\$env:HARNESS\_MSG="你好，介绍一下你自己"
...
\--- 已持久化(增量): sessions\\--G-dswork-duan-light-merge-lightharness--\cli\_xxx\session.jsonl ---
```
...
1. `翻译流`**（客户端.light L64-127）对增量缺键已有守卫**：`增量.包含("content") 且 增量["content"] != ""`。

   → 若根因在增量，需检查守卫**未覆盖**的路径（如 `调用["index"]`、`调用["id"]`、`调用["function"]["name"]`、

   `选择["delta"]` 本身为 None 等）。

2. **运行链**：`运行.py` → 委托 `light-merge` 编译器 `cli.light run` → 转译光明为 Python 执行
...
1. **定位并修复根因**：修正缺失键 /`空` 值的拼接（补 `.包含` 守卫、`!= 空` 判空、或安全取值）。
...
1. **如实交付**：若根因确实无法用假 SSE 复现、必须真实 API 验证，最多允许 2 次真实调用，
...
python 运行.py examples/test\_总入口.light

\# .env 位于项目根，需在项目根运行；密钥键名 OPENAI\_API\_KEY / OPENAI\_BASE\_URL / OPENAI\_MODEL
```
