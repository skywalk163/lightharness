...
[0.82] 对比：082_lightmerge基线_2026-09-18-010728.json  ⟵  082_lightmerge基线_2026-09-18-002428.json
...
> 补一笔如实记录：本轮**先后起过两次 gate，且重叠了一段时间**
> （第一次 00:57 出 `082_lightmerge基线_2026-09-18-005728.json`，第二次 01:07 出
> `..._010728.json`，都以 `..._002428.json` 为基线，结论一致：**新增红 0、已修复 1、门 PASS**）。
> 上面贴的是**最后一次（010728）**的完整输出。
> 重叠的原因是我想省时间提前发起了第二次，结果两边共抢同一份 `/tmp/r44-*` 副本、进度互相拖慢——
> 这就是下面 §五-3 那条「别同时发第二个远端任务」的由来，不是从文档里抄的，是踩出来的。见下。
...
2. **本机重跑一次要 25 分钟**：这也是 `--local-xml` 存在的理由，
   但正式回归时该花还得花。
3. **0.82 全量期间不能同时发第二个远端任务**：同一份 `/tmp/r44-*` 副本被串行占用。
   本轮实测踩到：两个 gate 重叠 → 进度点互相拖慢（单次 1427s 被拖到两次叠加约 35 分钟），
   且会多产出一份基线文件、把 `latest` 指针和门 JSON 冲成后跑完的那一份。
   结论：要么排队，要么等前一个结束再发。
