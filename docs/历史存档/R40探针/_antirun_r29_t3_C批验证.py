# -*- coding: utf-8 -*-
"""R29 任务3：C批（测试/生成+关键字 10条）CCW 逐条隔离验证。
...
C批条目：测试_生成问候语 生成分析 生成问候语 最终答案 应当 当然 除非 当前 终点 开启调试
输出：_task3_R29_C批验证_证据.json + 控制台摘要
"""
import os
import sys
import json
...
C_BATCH = ['测试_生成问候语', '生成分析', '生成问候语', '最终答案',
           '应当', '当然', '除非', '当前', '终点', '开启调试']
...
def main():
    ...
    missing = [w for w in C_BATCH if w not in base_ccw]
    assert not missing, 'C批条目不在CCW: %s' % missing

    print('=' * 72)
    print('R29 任务3：C批 CCW 逐条隔离验证（%d 条，语料 %d 文件）'
          % (len(C_BATCH), len(CORPUS)))
    print('=' * 72)
    ...
    # G3 基线（CS 基态）
    g3_src = {w: g3_forms(w) for w in C_BATCH}
    g3_base = {w: [token_seq(s) for s in ss] for w, ss in g3_src.items()}

    # G2 编译探针文件（每条目一个最小可编译文件）
    g2_dir = os.path.join(HARNESS, '_r29_c_g2')
    os.makedirs(g2_dir, exist_ok=True)
    g2_files = {}
    for w in C_BATCH:
        p = os.path.join(g2_dir, '_g2_%d.light' % C_BATCH.index(w))
    ...
    out = os.path.join(HARNESS, '_task3_R29_C批验证_证据.json')
    json.dump({'c_batch': C_BATCH, 'corpus_files': len(CORPUS),
               'base_err_files': base_errs, 'evidence': evidence},
              open(out, 'w', encoding='utf-8'), ensure_ascii=False, indent=1)
    deletable = [w for w in C_BATCH if evidence[w]['verdict'] == '可删']
    print('-' * 64)
    print('可删 %d 条: %s' % (len(deletable), '、'.join(deletable)))
    print('保留 %d 条: %s' % (len(C_BATCH) - len(deletable),
                              '、'.join(w for w in C_BATCH if w not in deletable)))
    print('证据文件: %s' % out)
