# -*- coding: utf-8 -*-
"""R37 任务4：全量反跑 + 回归验证 + 导入路径检查（合并任务1-2 后代码基线）。

用法：
  python _antirun_r37_final.py snapshot <out.json>          # §A 全语料 tokenize 快照
  python _antirun_r37_final.py compare <base.json>          # §A 对基线比对
  python _antirun_r37_final.py tests [out.json]             # §B 全量 examples/test_*.light rc 扫描
  python _antirun_r37_final.py imports                      # §C 导入路径检查
  python _antirun_r37_final.py all <out.json>               # §A快照+§C（§B 单独跑）
...
# ---------------------------------------------------------------- §C
def import_check():
    pats = ['从 代理循环 导入', '从 代理默认模型 导入', '代理循环', '代理默认模型']
    hits = {p: [] for p in pats}
    for f in CORPUS:
        if rel(f) in ('lightharness/src/代理循环.light', 'lightharness/stdlib/代理循环.light',
                      'lightharness/src/代理默认模型.light',
                      'lightharness/examples/test_R37_代理默认模型.light'):
