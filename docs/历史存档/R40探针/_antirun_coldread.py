# -*- coding: utf-8 -*-
# 任务1 反跑判据验证：把 src 关键逻辑改反 → 必须 rc != 0（红）→ 字节级恢复 → 必须绿。
# 判据 A：去掉 中断轮闭合器 合成 → test_会话冷读 未闭合轮合成断言红。
# 判据 B：时间下界去掉 nextafter 边界 → test_查询工具 亚毫秒边界断言红。
# 字节级备份/恢复 src：以二进制读写，恢复时写回原字节（含 BOM/换行不变）。
import io, os, sys, subprocess

ROOT = os.path.dirname(os.path.abspath(__file__))


def run_test(path):
    env = dict(os.environ)
    env['LIGHT_MERGE'] = r'G:\dswork\duan-light-merge\light-merge'
    p = subprocess.run([sys.executable, '运行.py', path],
                       cwd=ROOT, capture_output=True, text=True, encoding='utf-8', errors='replace')
    return p.returncode


cases = [
    # (src 文件, 原串, 反串, 目标测试, 说明)
    (os.path.join(ROOT, 'src', '会话冷读.light'),
     '设 合成表 为 中断轮闭合器(事件表)',
     '设 合成表 为 []  # 反跑：去掉中断轮合成 → 红',
     'examples/test_会话冷读.light',
     '冷读去掉中断轮闭合器合成 → 未闭合轮合成断言红'),
    (os.path.join(ROOT, 'src', '查询工具.light'),
     '返回 math.nextafter(时间戳["毫秒"], float("inf"))',
     '返回 时间戳["毫秒"]  # 反跑：时间下界去掉 nextafter → 红',
     'examples/test_查询工具.light',
     '查询工具时间下界去掉 nextafter → 亚毫秒边界断言红'),
]

ok = True
for full, old, new, test_path, desc in cases:
    with io.open(full, 'rb') as f:
        data = f.read()
    old_b = old.encode('utf-8')
    new_b = new.encode('utf-8')
    if old_b not in data:
        print('MISS:', desc)
        ok = False
        continue
    data2 = data.replace(old_b, new_b, 1)
    with io.open(full, 'wb') as f:
        f.write(data2)
    rc = run_test(test_path)
    with io.open(full, 'wb') as f:
        f.write(data)  # 字节级恢复
    rc2 = run_test(test_path)
    if rc == 0:
        print('反跑不红 FAIL:', desc, '(rc=0 应红)')
        ok = False
    else:
        print('反跑即红 PASS:', desc, '(rc=%d)' % rc)
    if rc2 != 0:
        print('恢复未绿 FAIL:', desc, '(rc=%d)' % rc2)
        ok = False
    else:
        print('恢复即绿 PASS:', desc)

print('ALL OK' if ok else 'HAS FAIL')
sys.exit(0 if ok else 1)
