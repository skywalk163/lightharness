# -*- coding: utf-8 -*-
"""_antirun_t1_子代理核心.py —— 第 16 轮任务 1 反跑判据（subagent 子代理核心 上）

对象：src/子代理核心.light（字节级备份 → 变异 → 跑测试 → 断言真红 → 恢复 → 断言真绿）
判据（3/3）：
  A. 目录列举丢结果（`目录条目表` 去掉遍历产出）→ 1y「条目表 与行一致」红；恢复 → 绿
  B. 空收件箱被误判有积压（`收件箱积压` 末句恒真）→ 6f「空箱无积压」红；恢复 → 绿
  C. 深度最大值选择反向（`读委托深度` 的 `头 > 运` → `头 < 运`）→ 3b/3e 红；恢复 → 绿
用法：python _antirun_t1_子代理核心.py
...
TARGET = os.path.join(BASE, "src", "子代理核心.light")
TEST = os.path.join(BASE, "examples", "test_子代理核心.light")
ENTRY = os.path.join(BASE, "运行.py")
PASS_LINE = "test_子代理核心 PASS"
...
def main():
    ...
    try:
        case("A 目录列举丢结果", A_OLD, A_NEW)
        case("B 空收件箱误判积压", B_OLD, B_NEW)
        case("C 深度最大值选择反向", C_OLD, C_NEW)
    finally:
        write_bytes(TARGET, original)
    ...
    print("=== 第 16 轮任务 1 反跑判据（subagent 子代理核心 上）===")
