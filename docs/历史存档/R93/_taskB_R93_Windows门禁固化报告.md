...
> 【R93-M 收口补充，见 `_taskM_R93_收口报告.md` §1/§6】M 路独立复核在本机被桌面应用吃到 99% CPU 时
> 抓到这两条**仍会红**（孤立 5 轮全红，签名「10s 内 pid 文件未出现…树根本没建好」），于是追加真修
> `[R93-M-BOOTSTRAP-WINDOW]`：等"树建好"的窗口 10s → 30s（**杀树断言一字未改**），修后同负载 3/3 绿。
> 销账结论不变，且证据比本路原销账更硬（最恶劣负载下亦绿）。
