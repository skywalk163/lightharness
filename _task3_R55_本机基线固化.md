...
> 🛑 **本节结论已被证伪（第56轮更正，原文保留以便追溯）**
>
> 下面这张表的「0.82 FreeBSD rc = 0 ✅」是**测量假阳性**，不是真实结果。
>
> **错在哪**：本节取证用的是 `运行.py examples/X.light 2>&1 | tail -6`，
> 取到的是管道最后一个命令 **`tail` 的退出码（恒为 0）**，而不是 `运行.py` 的真实 rc。
>
> **更正后的事实（第56轮任务2，主 agent 已独立复核）**：这 4 条在
> **本机 py3.13 / 0.82 py3.11 / 0.82 py3.12 三处环境全部 rc=1**，报错逐字节一致。
> 词法器在 py3.11 与 py3.12 下对这 4 个文件产出**逐字节相同的 token 流**
> （83 / 668 / 563 / 486 个 token，两版本完全一致）。
> → **它既不是平台差异，也不是 Python 版本差异，而是 light-merge 词法器的通用缺陷**，
> 与本节标题所称的「Windows 独占」相反。
>
> 详见 `_task2_R56_Windows独占红归因.md`。
> 通用方法教训：**任何需要取退出码的场景，禁止接 `| tail`/`| head` 管道**，
> 应写 `cmd > /tmp/o 2>&1; echo __RC__=$?`。

| example | 本机 Windows rc | 0.82 FreeBSD rc |
|---|---|---|
| test_R22_嵌入关键字冗余验证 | 1 ❌ | ~~0 ✅~~ → 实为 **1 ❌** |
| test_R26_词首并入反向 | 1 ❌ | ~~0 ✅~~ → 实为 **1 ❌** |
| test_R26_词首并入混合 | 1 ❌ | ~~0 ✅~~ → 实为 **1 ❌** |
| test_R27_词首并入反向 | 1 ❌ | ~~0 ✅~~ → 实为 **1 ❌** |
